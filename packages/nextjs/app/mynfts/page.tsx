...
const MyNFTs: NextPage = () => {
  ...
  const { data: dataReceived, loading: loadingReceived, error: errorReceived } = useQuery(GET_NFTS_RECEIVED, {
    client: mooveNFTClient,
    variables: { address: address },
    pollInterval: 5000,
  })

  const { data: dataSent, loading: loadingSent, error: errorSent } = useQuery(GET_NFTS_SENT, {
    client: mooveNFTClient,
    variables: { address: address },
    pollInterval: 5000,
  })
  ...
  if (loadingReceived || loadingSent) {
    return <div className="text-center text-white my-8">Loading My NFTs...</div>;
  }

  if (errorReceived || errorSent) {
    return <div className="text-center text-white my-8">{errorReceived?.message}</div>;
  }
